...
async function main() {
  ...
  const productsData = [
    ...
    // Hair Products
    {
      ...
      price: 380,
      stockQty: 30,
      brandId: brands['olaplex'].id,
      categoryId: categories['hair-products'].id,
      status: 'ACTIVE',
      volume: '100ml',
  ...
  for (const p of productsData) {
    ...
    if (!existing) {
      ...
      } else if (data.categoryId === categories['hair-products'].id) {
        placeholderImage = '/placeholders/hair.svg'
      }
